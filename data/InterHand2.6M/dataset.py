...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode, annot_subset):
        ...
        for aid in db.anns.keys():
            ...
            focal, princpt = (
                np.array(cameras[str(capture_id)]["focal"][str(cam)], dtype=np.float32),
                np.array(
            ...
            if (
                self.mode == "val" or self.mode == "test"
            ) and cfg.trans_test == "rootnet":
    ...
    def __getitem__(self, idx):
        ...
        img, joint_coord, joint_valid, hand_type, inv_trans = augmentation(
            img, bbox, joint_coord, joint_valid, hand_type, self.mode, self.joint_type
        )
    ...
    def evaluate(self, preds):
        ...
        for n in range(sample_num):
            data = gts[n]
            bbox, cam_param, joint, gt_hand_type, hand_type_valid = (
                data["bbox"],
                data["cam_param"],
                data["joint"],
                data["hand_type"],
            ...
            for j in range(self.joint_num * 2):
                pred_joint_coord_img[j, :2] = trans_point2d(
                    pred_joint_coord_img[j, :2], inv_trans[n]
                )
            # restore depth to original camera space
            pred_joint_coord_img[:, 2] = (
                pred_joint_coord_img[:, 2] / cfg.output_hm_shape[0] * 2 - 1
            ) * (cfg.bbox_3d_size / 2)
            ...
            if (
                ...
                pred_rel_root_depth = (
                    preds_rel_root_depth[n] / cfg.output_root_hm_shape * 2 - 1
                ...
                gt_rel_root = (
                    gt_joint_coord[self.root_joint_idx["left"]]
                    - gt_joint_coord[self.root_joint_idx["right"]]
                )
            ...
            for h in ("right", "left"):
                ...
                gt_joint_coord[self.joint_type[h]] = (
                    gt_joint_coord[self.joint_type[h]]
                    - gt_joint_coord[self.root_joint_idx[h], None, :]
                )
                ...
                if joint_valid[j]:
                    if gt_hand_type == "right" or gt_hand_type == "left":
                        mpjpe_sh[j].append(
                            np.sqrt(
                                np.sum(
                                    (pred_joint_coord_cam[j] - gt_joint_coord[j]) ** 2
                                )
                            )
                        )
                    else:
            ...
            if vis:
                ...
                vis_3d_keypoints(
                    pred_joint_coord_cam, joint_valid, self.skeleton, filename
                )
