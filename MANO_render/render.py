...
with open(
    osp.join(annot_root_path, subset, "InterHand2.6M_" + split + "_MANO.json")
) as f:
    mano_params = json.load(f)
with open(
    osp.join(annot_root_path, subset, "InterHand2.6M_" + split + "_camera.json")
) as f:
    cam_params = json.load(f)
with open(
    osp.join(annot_root_path, subset, "InterHand2.6M_" + split + "_joint_3d.json")
) as f:
    joints = json.load(f)
...
for img_path in tqdm(img_path_list):
    ...
    for hand_type in ("right", "left"):
        ...
        # fitting error
        fit_err = get_fitting_error(
            mesh,
            ih26m_joint_regressor,
            cam_params,
            joints,
            hand_type,
            capture_idx,
        ...
        save_obj(
            mesh,
            mano_layer[hand_type].faces,
            osp.join(
                save_path, img_path.split("/")[-1][:-4] + "_" + hand_type + ".obj"
            ),
        )
        ...
        if prev_depth is None:
            ...
            render_mask = valid_mask * np.logical_or(
                depth < prev_depth, prev_depth == 0
            )
            img = rgb * render_mask + img * (1 - render_mask)
